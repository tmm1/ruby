...
heap_calc_increment(rb_objspace_t *objspace)
{
    size_t size = (size_t)(heap_used * initial_growth_factor);

    if (initial_growth_max > 0) {
	size_t limit = (size_t)(heap_used + initial_growth_max/HEAP_OBJ_LIMIT);
...
{
    size_t next_heap_length = heap_calc_increment(objspace);
